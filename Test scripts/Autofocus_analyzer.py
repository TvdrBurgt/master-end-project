...
path = r'W:/staff-groups/tnw/ist/do/projects/Neurophotonics/Brinkslab/Data/Thijs/feedback labtest #15'

autofocustimestamp = r'autofocus_2021-12-17_13-57-46'
...
plt.xlabel(r'Micromanipulator height (in $\mu$m)')
plt.ylabel(r'Variance of Laplacian (a.u.)')

# plt.figure()
# plt.title('Sharpness function differences')
# plt.scatter(positions[0:-1], np.diff(penalties), c=np.linspace(0,1,len(positions[0:-1])), cmap='rainbow', label="f'(x)", marker='2')
# plt.xlabel(r'Micromanipulator height (in $\mu$m)')
# plt.ylabel(r'difference in Variance of Laplacian (a.u.)')
