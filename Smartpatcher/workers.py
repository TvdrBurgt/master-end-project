...
class Worker(QObject):
    ...
    def request_imagexygrid(self):
        ...
        for i in range(0, 26):
            for j in range(0, 26):
                path = np.array([i*stepsize, j*stepsize, 0])
                x,y,z = account4rotation(origin=reference, target=reference+path)
                micromanipulator.moveAbs(x, y, z)
                for k in ['a','b']:
                    if k == 'b':
    ...
    def request_imagezstack(self):
        ...
        for i in range(0,201):
            ...
            snap = camera.snap()
            io.imsave(save_directory+'_imagezstack_'+'Z%d'%(i*stepsize)+'.tif', snap, check_contrast=False)
            x2,y2,z2 = micromanipulator.getPos()
