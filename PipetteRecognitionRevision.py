...
import numpy as np
from skimage import io, filters, feature, transform, morphology
...
# load tiff
left = r"C:\Users\tvdrb\Desktop\data\grid 0 0 0.tif"
right = r"C:\Users\tvdrb\Desktop\data\grid 20 0 0.tif"
L = io.imread(left)
R = io.imread(right)
...
diameter = 30       # pipette tip diameter (in pixels)
...
def detectPipettetip(L, R, upper_angle, lower_angle, diameter, blursize=15, 
    ...
    LB = filters.gaussian(L, 1)
    RB = filters.gaussian(R, 1)
    
    # Image subtraction
    print('Ib)\t Image subtraction...')
    IB = LB - RB
    
    # Canny edge detection
    print('II)\t Canny edge detection...')
    BW = feature.canny(IB, sigma=1, low_threshold=0.99, high_threshold=0, use_quantiles=True)
    
    # Dilation
    print('IIb) Dilating the binary image...')
    # Make lines thicker and more easily detectable, comes at a cost of accuracy
    BW = morphology.binary_dilation(BW, selem=[[1,1,1],[1,1,1],[1,1,1]])
...
if __name__ == '__main__':
    detectPipettetip(L, R, upper_angle, lower_angle, diameter, plotflag=True)
